      ...
      <div className="grid gap-5 md:grid-cols-3">
        {parceiros.map((item) => (
          <article key={item.id} className="card p-6">
            ...
            {item.site_url && <a className="mt-4 inline-block text-sm font-bold text-asp-green" href={item.site_url} target="_blank" rel="noreferrer">Visitar site</a>}
          </article>
        ))}
      </div>
      <div className="mt-10 rounded-lg bg-asp-soft p-8">
        <h2 className="text-2xl font-bold">Empresas podem apoiar campanhas, eventos e necessidades pontuais.</h2>
        <a className="btn-primary mt-5" href={whatsappLink(config?.whatsapp, 'Olá! Minha empresa gostaria de apoiar a ONG.')} target="_blank" rel="noreferrer">Falar sobre parceria</a>
      </div>
    </main>
  );
}
